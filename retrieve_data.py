...
if "links.csv" not in os.listdir("../data/"):
...
page = 17
while len(df_links) < 3000:
    try:
        print(page)
        request = requests.get(f"https://phishtank.org/phish_search.php?page={page}&active=y&verified=u")
        soup = BeautifulSoup(request.text, "html.parser")
        try:
            table = soup.find_all("table")[-1]
        except IndexError:
            print("Table on page not found")
            page += 1
            continue

        for tr in table.find_all("tr")[1:]:
            link = tr.find_all("td")[1].contents[0].strip().replace('www.', '').replace('...', '.')\
                                                                                .replace('..', '.')
